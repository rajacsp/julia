...
function match_logs(f, patterns...; match_mode::Symbol=:all, kwargs...)
    ...
    if match_mode == :all
        didmatch = length(logs) == length(patterns) &&
            all(isfound(p, l) for (p,l) in zip(patterns, logs))
    elseif match_mode == :any
        didmatch = all(any(isfound(p, l) for l in logs) for p in patterns)
    end
    didmatch,logs,value
end
...
logfield_contains(a, r::Regex) = isfound(r, a)
logfield_contains(a::Symbol, r::Regex) = isfound(r, String(a))
logfield_contains(a::LogLevel, b::Symbol) = a == parse_level(b)
logfield_contains(a, b::Ignored) = true

function isfound(pattern::Tuple, r::LogRecord)
