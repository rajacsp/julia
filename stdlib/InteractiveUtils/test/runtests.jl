...
str = String(take!(iob))
@test isfound(Base.text_colors[Base.warn_color()], str)
...
let v = repr(varinfo(_test_varinfo_))
    @test isfound("| x              |   8 bytes | Float64 |", v)
end
...
@testset "versioninfo" begin
    ...
    mktempdir() do dir
        withenv("JULIA_PKGDIR" => dir) do
            ...
            @test isfound("Environment:", ver)
            @test isfound("Package Status:", ver)
            @test isfound("no packages installed", ver)
...
if Sys.ARCH === :x86_64 || isfound(ix86, string(Sys.ARCH))
    function linear_foo()
        x = 4
        y = 5
    end
