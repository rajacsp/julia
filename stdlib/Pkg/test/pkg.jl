...
temp_pkg_dir() do
    ...
    begin
        try
            ...
            @test isfound("Cannot clone Example from notarealprotocol://github.com/JuliaLang/Example.jl.git", ex.msg)
        end
    end
    ...
    begin
        ...
        @test isfound("Building BuildFail", msg)
        @test !isfound("Build failed for BuildFail", msg)
        ...
        @test isfound("Building BuildFail", msg)
        @test isfound("Build failed for BuildFail", msg)
        @test isfound("Pkg.build(\"BuildFail\")", msg)
        @test isfound("Throw build error", msg)
    end
    ...
    let package = "Example"
        ...
        @test isfound("$package is not an installed package", msg)
        @test !isfound("signal (15)", msg)
    end
    ...
    @test "" == capture_stdout() do
        ...
        @test all(!isfound("updated but were already imported", l.message) for l in logs)
        ...
        @test any(isfound((:info, r"Upgrading ColorTypes: v0\.2\.2 => v\d+\.\d+\.\d+"), l) for l in logs)
        @test any(isfound((:info, r"Upgrading Compat: v0\.7\.18 => v\d+\.\d+\.\d+"), l) for l in logs)
        @test !any(isfound((:info, r"Upgrading Colors"), l) for l in logs)
    ...
    let package = "Example"
        ...
        @test isfound(Regex("- $package.*Restart Julia to use the updated versions","s"), msg)
    end
    ...
    let package = "StartupFile"
        ...
        withenv((Sys.iswindows() ? "USERPROFILE" : "HOME") => home) do
            ...
            @test isfound("JULIA_RC_LOADED defined false", msg)
            @test isfound("Main.JULIA_RC_LOADED defined false", msg)

            msg = read(`$(Base.julia_cmd()) --startup-file=yes -e $code`, String)
            @test isfound("JULIA_RC_LOADED defined false", msg)
            @test isfound("Main.JULIA_RC_LOADED defined true", msg)
            ...
            msg = read(`$(Base.julia_cmd()) --startup-file=yes -e $code`, String)
            @test isfound("JULIA_RC_LOADED defined true", msg)
            @test isfound("Main.JULIA_RC_LOADED defined true", msg)
        end
    end
