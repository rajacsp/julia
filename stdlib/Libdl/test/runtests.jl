...
@test length(filter(dlls) do dl
        return isfound(Regex("^libjulia(?:.*)\\.$(Libdl.dlext)(?:\\..+)?\$"), basename(dl))
...
@test !isfound("foo.$(Libdl.dlext).0.$(Libdl.dlext)", err.value.msg)
err = @test_throws ErrorException Libdl.dlopen("./foo.$(Libdl.dlext).0.22.1")
@test !isfound("foo.$(Libdl.dlext).0.22.1.$(Libdl.dlext)", err.value.msg)
