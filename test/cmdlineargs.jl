...
let exename = `$(Base.julia_cmd()) --sysimage-native-code=yes --startup-file=no`
    ...
    let code = writereadpipeline("code_llvm(stdout, +, (Int64, Int64), false, true)", `$exename -g0`)
        @test code[2]
        code = code[1]
        @test isfound("llvm.module.flags", code)
        @test !isfound("llvm.dbg.cu", code)
        @test !isfound("int.jl", code)
        @test !isfound("Int64", code)
    end
    ...
    for nonexist_image in (
        ...
        let err = Pipe(),
            ...
            let s = read(err, String)
                @test isfound("ERROR: could not load library \"$nonexist_image\"\n", s)
                @test !isfound("Segmentation fault", s)
                @test !isfound("EXCEPTION_ACCESS_VIOLATION", s)
            end
            @test !success(p)
...
for precomp in ("yes", "no")
    ...
    @test !success
    @test out == ""
    @test isfound("include_relative(::Module, ::String) at $(joinpath(".", "loading.jl"))", bt)
