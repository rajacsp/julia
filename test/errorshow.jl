...
@test isfound("method_c6(; x)$cfile$(c6line + 1) got unsupported keyword argument \"y\"", error_out)
@test isfound("method_c6(!Matched::Any; y)$cfile$(c6line + 2)", error_out)
@test isfound("method_c6(::Any; y)$cfile$(c6line + 2) got unsupported keyword argument \"x\"", error_out1)
@test isfound("method_c6_in_module(; x)$cfile$(c6mline + 2) got unsupported keyword argument \"y\"", error_out2)
@test isfound("method_c6_in_module(!Matched::Any; y)$cfile$(c6mline + 3)", error_out2)
@test isfound("method_c6_in_module(::Any; y)$cfile$(c6mline + 3) got unsupported keyword argument \"x\"", error_out3)
...
let no_kwsorter_match, e
    ...
    @test isfound(r"no method matching.+\(; y=1\)", sprint(showerror, e))
end
...
let
    ...
    @test !isfound("::$(curmod_prefix)InstanceType11007", err_str)
    @test isfound("::$(curmod_prefix)InvokeType11007", err_str)
end
...
@test isfound("import Base.+", err_str)

err_str = @except_str Float64[](1) MethodError
@test !isfound("import Base.Array", err_str)
...
let
    ...
    @test isfound("row vector", err_str)
    @test isfound("column vector", err_str)
end

struct TypeWithIntParam{T <: Integer} end
let undefvar
    err_str = @except_strbt sqrt(-1) DomainError
    @test isfound("Try sqrt(Complex(x)).", err_str)
    err_str = @except_strbt 2^(1-2) DomainError
    @test isfound("Cannot raise an integer x to a negative power -1", err_str)
    err_str = @except_strbt (-1)^0.25 DomainError
    @test isfound("Exponentiation yielding a complex result requires a complex argument", err_str)
    A = zeros(10, 10)
    A[2,1] = 1
    A[1,2] = -1
    err_str = @except_strbt eigmax(A) DomainError
    @test isfound("DomainError with [0.0 -1.0 …", err_str)
...
let err_str,
    ...
    @test isfound("MethodError: no method matching Bool()", err_str)
    err_str = @except_str :a() MethodError
    @test isfound("MethodError: objects of type Symbol are not callable", err_str)
    err_str = @except_str EightBitType() MethodError
    @test isfound("MethodError: no method matching $(curmod_prefix)EightBitType()", err_str)
    err_str = @except_str i() MethodError
    @test isfound("MethodError: objects of type $(curmod_prefix)EightBitType are not callable", err_str)
    err_str = @except_str EightBitTypeT() MethodError
    @test isfound("MethodError: no method matching $(curmod_prefix)EightBitTypeT()", err_str)
    err_str = @except_str EightBitTypeT{Int32}() MethodError
    @test isfound("MethodError: no method matching $(curmod_prefix)EightBitTypeT{Int32}()", err_str)
    err_str = @except_str j() MethodError
    @test isfound("MethodError: objects of type $(curmod_prefix)EightBitTypeT{Int32} are not callable", err_str)
    err_str = @except_str FunctionLike()() MethodError
    @test isfound("MethodError: no method matching (::$(curmod_prefix)FunctionLike)()", err_str)
    err_str = @except_str [1,2](1) MethodError
    @test isfound("MethodError: objects of type Array{$Int,1} are not callable\nUse square brackets [] for indexing an Array.", err_str)
    # Issue 14940
    err_str = @except_str randn(1)() MethodError
    @test isfound("MethodError: objects of type Array{Float64,1} are not callable", err_str)
end
@test repr("text/plain", FunctionLike()) == "(::$(curmod_prefix)FunctionLike) (generic function with 0 methods)"
@test isfound(r"^@doc \(macro with \d+ method[s]?\)$", repr("text/plain", getfield(Base, Symbol("@doc"))))
...
let err, buf = IOBuffer()
    ...
    @test isfound("Closest candidates are:", String(take!(buf)))
end
...
@testset "closest candidates kwarg #9965" begin
    ...
    @test isfound("got unsupported keyword argument \"w\"", String(take!(io)))
end
...
let
    ...
    @test !isfound("where T at sysimg.jl", sprint(showerror, method_error))
...
@testset "show for manually thrown MethodError" begin
    ...
    @test !isfound("The applicable method may be too new", str)
    ...
    str = sprint(Base.showerror, ex1)
    @test startswith(str, "MethodError: no method matching f21006(::Tuple{})")
    @test isfound("The applicable method may be too new: running in world age $(ex1.world)", str)
