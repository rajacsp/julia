...
let a = Any[]
    ...
    @test a == [1, 6, 3]
    @test isfound("x + x", string(code_lowered(f23168, (Vector{Any},Int))))
    @test isfound("2 * x", string(Base.uncompressed_ast(first(methods(f23168)))))
    @test isfound("2 * x", string(code_lowered(f23168, (Vector{Any},Int), generated=false)))
    @test isfound("(Base.add_int)(x, x)", string(code_typed(f23168, (Vector{Any},Int))))
end
