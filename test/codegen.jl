...
if opt_level > 0
    ...
    @test !isfound(" call ", get_llvm(identity, Tuple{String}))
end
...
if opt_level > 0
    ...
    @test !isfound(" call ", get_llvm(jl_string_ptr, Tuple{String}))
    s = "aaa"
...
if opt_level > 0
    @test !isfound("%gcframe", get_llvm(pointer_not_safepoint, Tuple{}))
    compare_large_struct_ir = get_llvm(compare_large_struct, Tuple{typeof(create_ref_struct())})
    @test isfound("call i32 @memcmp", compare_large_struct_ir)
    @test !isfound("%gcframe", compare_large_struct_ir)

    @test isfound(Tuple{}, get_llvm(MutableStruct), "jl_gc_pool_alloc")
    breakpoint_mutable_ir = get_llvm(breakpoint_mutable, Tuple{MutableStruct})
    @test !isfound("%gcframe", breakpoint_mutable_ir)
    @test !isfound("jl_gc_pool_alloc", breakpoint_mutable_ir)

    breakpoint_badref_ir = get_llvm(breakpoint_badref, Tuple{MutableStruct})
    @test !isfound("%gcframe", breakpoint_badref_ir)
    @test !isfound("jl_gc_pool_alloc", breakpoint_badref_ir)

    breakpoint_ptrstruct_ir = get_llvm(breakpoint_ptrstruct, Tuple{RealStruct})
    @test !isfound("%gcframe", breakpoint_ptrstruct_ir)
    @test !isfound("jl_gc_pool_alloc", breakpoint_ptrstruct_ir)
end
...
if opt_level > 0
    ...
    @test !isfound("%gcframe", get_llvm(g24108, Tuple{B24108}))
end
...
@test isfound("!dereferenceable_or_null", get_llvm_noopt(foo24632, (Bool,), false))
@test !isfound("!nonnull", get_llvm_noopt(foo24632, (Bool,), false))
