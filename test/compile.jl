...
try
    ...
    @test_warn "ERROR: LoadError: Declaring __precompile__(false) is not allowed in files that are being precompiled.\nStacktrace:\n [1] __precompile__" try
        ...
        isfound("__precompile__(false)", exc.msg) && rethrow(exc)
    end
    ...
    @test_warn "ERROR: LoadError: break me\nStacktrace:\n [1] error" try
        ...
        isa(exc, ErrorException) || rethrow(exc)
        isfound("ERROR: LoadError: break me", exc.msg) && rethrow(exc)
    end
    ...
    try
        ...
        let fname = tempname()
            try
                ...
                @test isfound(Regex("Replacing module `$Test_module`"), read(fname, String))
            finally
                rm(fname, force=true)
            end
        end
